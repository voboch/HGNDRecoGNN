...
$0.7 \lesssim \Ekin \lesssim 5$~GeV.  The three \SMASH{} datasets
show per-cluster closure $C_c = 1.25 \pm 0.03$ (spread 5\%) at the
smoke scale, uniform across $\Usym$; the per-neutron closure
$C_\nu$ spreads 0.65--0.85 and is dominated by the physical
clusters-per-neutron ratio $k(\Usym)$
(Sec.~\ref{sec:closure}).
\end{abstract}
...
on the smoke-scale sensitivity subsets (matched 4-shard subsets of
each dataset, HPC checkpoint of Sec.~\ref{sec:performance},
pooled-scaler feature transform of Sec.~\ref{sec:systematics}).
The per-cluster closure is uniform across the three datasets at
$1.25 \pm 0.03$ (spread 5\%), confirming that the classifier +
regression pipeline is dataset-independent within statistics at this
scale.  All datasets over-recover slightly ($C_c > 1$) because the
efficiency correction inflates true positives but does not subtract
the small classifier background leak; this bias is treated as a
threshold-dependent systematic in Sec.~\ref{sec:systematics} item
\ref{sys:binning}.  The per-neutron closure $C_\nu$ spreads much more
widely (0.65--0.85) because $k$ varies physically across datasets
(0.51--0.69): \texttt{defaultSpot}'s higher effective neutron density
increases the multi-neutron cluster-merger probability by
$\sim 40\%$, and this alone accounts for essentially the whole
per-neutron closure spread.  Physics-quality sensitivity to $\Usym$
therefore requires either
(i) a differentiable clustering step that resolves multi-neutron
overlaps into separate cluster candidates, or
(ii) an unfolding pass over the response matrix
$M \otimes k^{-1}(\Usym)$ that treats cluster-merger as a
$\Usym$-dependent efficiency factor.  Both are deferred to
Sec.~\ref{sec:summary}.

\begin{table}[!t]
  \centering
  \caption{Closure decomposition at $t = 0.5$, smoke-scale HPC-checkpoint
    evaluation of the three \SMASH{} datasets on matched 4-shard
    subsets ($\sim 1600$--$2000$ events each).  Both efficiency-basis
    conventions of Sec.~\ref{sec:sensitivity} are quoted; a Tikhonov
    detector-matrix unfold agrees with the $E_{\text{pred}}$-basis
    correction to $\pm 3\%$ (\texttt{closure\_unfold} column of
    \texttt{results/sensitivity\_hpc\_pooled\_smoke\_fixed/e\_pred/sensitivity\_summary.csv}).}
  \label{tab:closure}
  \begin{tabular}{lrrrrr}
    \toprule
    dataset & $\Usym$ (MeV) & $k$ & $C_c$ (t=0.5) & $C_\nu$ (t=0.5) & $C_\nu = k C_c$ \\
    \midrule
    \texttt{zeroSpot}    & 0   & 0.687 & 1.23 & 0.846 & 0.845 \\
    \texttt{defaultSpot} & 18  & 0.511 & 1.27 & 0.647 & 0.649 \\
    \texttt{bigSpot}     & 90  & 0.663 & 1.29 & 0.853 & 0.855 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!t]
  \centering
  \caption{Purity-locked closure at $\pi = 0.7$ per dataset, smoke
    scale, HPC checkpoint. The $t^\star$ column is the per-dataset
    threshold that hits the target purity; $\varepsilon_{\text{cl}}$
    is the resulting cluster efficiency. Note that the per-neutron
    $C_\nu$ spread (max$-$min) widens from 0.206 at $t = 0.5$
    (Table~\ref{tab:closure}) to 0.244 here, consistent with the
    $k(\Usym)$-dominated interpretation.
    Source: \texttt{results/sensitivity\_hpc\_pooled\_smoke\_purityLocked/purity\_locked\_pi70.csv}.}
  \label{tab:purity_locked}
  \begin{tabular}{lrrrr}
    \toprule
    dataset & $t^\star$ & $\varepsilon_{\text{cl}}$ & $C_c$ & $C_\nu$ \\
    \midrule
    \texttt{zeroSpot}    & 0.333 & 0.674 & 1.44 & 0.967 \\
    \texttt{defaultSpot} & 0.303 & 0.773 & 1.40 & 0.736 \\
    \texttt{bigSpot}     & 0.313 & 0.710 & 1.47 & 0.980 \\
    \bottomrule
  \end{tabular}
\end{table}

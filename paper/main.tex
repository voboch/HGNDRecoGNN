...
\title{A heterogeneous graph-neural-network reconstruction of the
       Highly-Granular Time-of-Flight Neutron Detector at \BMN{}\\
       and its calibration across three
       nuclear-symmetry-potential simulations}
...
\begin{abstract}
We present a heterogeneous graph-neural-network (GNN) reconstruction
algorithm for the Highly-Granular Time-of-Flight Neutron Detector
(\HGND{}) of the fixed-target \BMN{} experiment at JINR.  The
detector concept, geometry, and time-of-flight response follow
Morozov \emph{et~al.}~\cite{Morozov2024HGND}.  Compared with the
two-network prototype reported there, the pipeline operates
cluster-wise on a single hetero-GNN that jointly produces a neutron
score, an energy prediction, and a link-classification output on
each candidate cluster; it is trained end-to-end on
\DCM{} + \GEANT{} Bi+Bi and \SMASH{} Xe+Cs simulations.
On the \DCM{} Xe+CsI test split (97{,}104 held-out events) the
checkpoint (\texttt{net\_default}, $H=512$, $L=8$, 20 epochs) reaches
cluster ROC AUC 0.944 and, at a signal-purity-locked working point
of $\pi = 0.702$ ($t^\star_{\text{DCM}} = 0.305$), cluster efficiency 0.739 and
per-event neutron detection efficiency 0.844 (Table~\ref{tab:eff_ekin}),
matching the prototype benchmarks of Ref.~\cite{Morozov2024HGND}; the
relative energy resolution is below 25\% for $1 \lesssim \Ekin \lesssim
3$~GeV and grows to $\sim 30\%$ toward the acceptance edges.
As a pipeline calibration we evaluate the same
checkpoint on three \SMASH{} samples with different values of the
nuclear symmetry potential ($\Usym = 0$, $18$, $90$~MeV) and
introduce a decomposition of the closure metric into a per-cluster
pipeline term and a physical clusters-per-neutron ratio
(Sec.~\ref{sec:closure}); at full statistics the per-cluster closure
$C_c$ is uniform across the three samples to $1.0\%$ at $t = 0.5$
and $0.62\%$ at the purity-locked point.
The associated per-neutron sensitivity to $\Usym$ is
\emph{provisional}: it is folded with the same MC-truth definition
used to train the network, and the SMASH samples underlying the
three-dataset comparison have an upstream simulation bug reported
after the analysis was frozen for this draft.  A physics-quality
symmetry-potential measurement is deferred to a follow-up paper on
re-simulated data (Sec.~\ref{sec:summary}).
\end{abstract}
...
accessed~\cite{Sorensen2024EoS, Burgio2020NuclearMatter}.
...
value of the nuclear symmetry potential.  The beam energy is
$0.93$~AGeV below the 2023 \BMN{} first Xe+CsI physics run
($3.8$~AGeV) used to fix the DCM sample above; this offset is
deliberate --- the \SMASH{} points were chosen to bracket the
maximum-density working point where transport models predict the
largest $\Usym$ signal~\cite{Sorensen2024EoS, LiChen2008SymEnergy}, and
the reconstruction pipeline is dataset-agnostic within the
$0.3 \lesssim \Ekin \lesssim 4$~GeV \HGND{} acceptance so training
transfers without retraining.  The three datasets are:
\begin{center}
  \begin{tabular}{@{}lll@{}}
    \toprule
  ...
  \end{tabular}
\end{center}
All three datasets are fully preprocessed at
$1.8$--$2.2 \times 10^{6}$ graphs each on cHARISMa
(preprocess jobs 4284505 and 4290339).  Smoke-scale subsets
(\num{4000} graphs each) are retained for laptop validation of
the pipeline (see \texttt{tests/test\_sensitivity\_pipeline.py}).
Full-scale preprocessing runs on 16-CPU-worker Rocky Linux 9 nodes
and takes $2$--$3$~h per dataset (see
...
Total parameter count $1.248 \times 10^{7}$ (12{,}479{,}173
learnable weights, verified from the HPC checkpoint state\_dict).
...
Training is performed on a single NVIDIA V100 (32 GB) node with
the Adam optimizer (learning rate $10^{-3}$, weight decay
$10^{-5}$), \texttt{MultiStepLR} milestones at epochs
$\{10, 15, 25\}$ ($\gamma=0.1$), batch size 512, and a 50/50
train/test split (seed 42).  Full-scale training runs for 20 epochs
and completes in 1~h~39~min wall time on the cHARISMa cluster
(reference job 4278194).  Model checkpoints, training curves, and
per-run manifests are archived under \texttt{checkpoints/} of the
analysis repository.
...
(combined top+bottom).  On the \texttt{defaultSpot} test split
(97{,}104 held-out events, HPC checkpoint of
Sec.~\ref{sec:reproducibility}) the cluster ROC has
$\text{AUC}_{\text{cl}} = 0.944$
(hit-level 0.956, edge-level 0.949; average precision 0.807).
At the fixed classifier operating point $t = 0.5$ the pipeline
reaches cluster purity $\pi = 0.813$, cluster efficiency
$\varepsilon_{\text{cl}} = 0.632$, and per-event neutron detection
efficiency $\varepsilon_{\text{ev}} = 0.744$ (F1 = 0.711).
At the purity-locked operating point $t^\star = 0.305$
($\pi = 0.702$, from the fine threshold scan of
Sec.~\ref{sec:gnn:baseline}) the cluster efficiency is
$\varepsilon_{\text{cl}} = 0.739$ and the per-event neutron
detection efficiency $\varepsilon_{\text{ev}} = 0.844$, matching
the value reported at the \BMN{} analysis-and-detector
meeting~\cite{Bocharnikov2026Dombay}.  (The corresponding
purity-locked working point on the full-statistics
\texttt{defaultSpot}~v2 SMASH cache is
$t^\star = 0.310, \pi = 0.701$, Table~\ref{tab:purity_locked};
the difference reflects the DCM$\to$SMASH sample change, not a
pipeline instability.)  All quantities are computed
from the prediction pickles under
\texttt{notebooks/results/hpc\_defaultSpot\_v2\_seed42/}
(cell 12 of \texttt{notebooks/evaluate\_hpc\_checkpoint.ipynb}).
%%   notebooks/results/sensitivity_full/performance_defaultSpot.png

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figs/performance_roc}
...
At the purity-locked operating point the per-cluster efficiency vs.\
kinetic energy tracks the Morozov 2024
$\varepsilon(\Ekin)$ curve~\cite{Morozov2024HGND}, saturating
at $\varepsilon_{\text{cl}} \gtrsim 0.80$ above 2 GeV and dropping
below 0.5 in the near-threshold [0.3, 1.0) GeV range that dominates
the reconstruction uncertainty (Table~\ref{tab:eff_ekin}).

\begin{table}[!t]
  \centering
  \caption{Cluster-level neutron detection efficiency at the
    purity-locked operating point ($t^\star = 0.305$, $\pi = 0.702$),
    HPC checkpoint on the \texttt{defaultSpot} test split (97{,}104
    events). Errors are $1\sigma$ Wilson binomial intervals. The
    below-1 GeV bins are the dominant contribution to the residual
    inefficiency. Reproduced by
    \texttt{notebooks/evaluate\_hpc\_checkpoint.ipynb} cell 16
    (source CSV
    \texttt{notebooks/results/hpc\_defaultSpot\_v2\_seed42/eff\_ekin\_purity70.csv}).}
  \label{tab:eff_ekin}
  \begin{tabular}{lrr}
    \toprule
    $\Ekin$ range (GeV) & signal clusters & $\varepsilon_{\text{cl}}$ \\
    \midrule
    $[0.3, 0.5)$ & 1{,}200  & $0.328 \pm 0.014$ \\
    $[0.5, 1.0)$ & 5{,}103  & $0.547 \pm 0.007$ \\
    $[1.0, 2.0)$ & 13{,}838 & $0.737 \pm 0.004$ \\
    $[2.0, 3.0)$ & 11{,}020 & $0.824 \pm 0.004$ \\
    $[3.0, 5.0)$ & 8{,}012  & $0.807 \pm 0.005$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
on a logarithmic colour scale.  Linearity is within $\sim 25\%$ for
$1 \lesssim \Ekin \lesssim 3$~GeV and degrades to $\sim 30\%$
toward the acceptance edges (positive bias at $\Ekin < 1$~GeV,
negative bias at $\Ekin > 5$~GeV); the model compensates for the
pure-ToF overestimate visible at $\Ekin > 2$~GeV in
Eq.~\eqref{eq:etof} but does not fully correct the low-energy
overshoot (compare Ref.~\cite{Morozov2024HGND}, Fig.~13).  The
relative energy resolution is $\sigma_E/E \lesssim 25\%$ over
$1 \le \Ekin \le 3$~GeV and grows toward the acceptance edges.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figs/ereco_vs_etrue}
...
month of Xe+CsI running.  These design-run statistics are
$\sim 6$ orders of magnitude above the smoke-scale analysis samples
of Sec.~\ref{sec:sensitivity} ($\sim 1600$--$2000$ events per
dataset) and $\sim 4$ orders of magnitude above the full \SMASH{}
sensitivity samples ($500\,000$ events per dataset,
Sec.~\ref{sec:simulation}); a fully-statistics reproduction of the
current sensitivity study on real data is therefore not statistics-
limited.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figs/multiplicity}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figs/mctruth_spectra}
...
be tracked in the closure test below.  The efficiency is quoted
against the measured $E_{\text{pred}}$ so that its bin index matches
the $\Nreco(E_{\text{pred}})$ numerator of Eq.~\eqref{eq:solved};
the physics-truth version $\eff(E_{\text{true}})$ is retained as a
diagnostic in the reproducibility appendix.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figs/efficiency_vs_ekin}
  \caption{Per-dataset cluster efficiency $\varepsilon_{\text{cl}}
    (E_{\text{pred}})$ at classifier threshold $t = 0.5$ for the
    three \SMASH{} samples, with Wilson binomial score intervals.
    The $E_{\text{pred}}$ binning matches the numerator of
    Eq.~\eqref{eq:solved} by construction.
  }
  \label{fig:eff}
\end{figure}
...
Poisson error on $\Nreco$ and the Wilson interval on $\eff$.

Both quantities in Eq.~\eqref{eq:solved} are binned by the
\emph{measured} predicted energy $E_{\text{pred}}$: $\Nreco$ counts
clusters above threshold in each $E_{\text{pred}}$ bin, and
$\eff(E_{\text{pred}}, t) = P(s > t \mid y = 1, E_{\text{pred}} \in \text{bin})$
is estimated from signal clusters in the same $E_{\text{pred}}$ bin.
Using the physics-truth energy $E_{\text{true}}$ for the denominator
while the numerator uses $E_{\text{pred}}$ mis-aligns the two spectra
whenever the energy regression is biased (Sec.~\ref{sec:performance}),
which trivially violates closure and was the leading source of the
$O(3)\%$ closure deficit in early smoke runs (see
Sec.~\ref{sec:systematics} item~\ref{sys:binning}).  A physics
efficiency curve $\eff(E_{\text{true}}, t)$ is still reported in
Fig.~\ref{fig:eff}; it is used only for interpretation and never as
the denominator of Eq.~\eqref{eq:solved}.

As a cross-check we additionally unfold the reconstructed
$\Nreco(E_{\text{pred}})$ spectrum with the detector response
matrix
$M_{ij} = P(E_{\text{pred}} \in \text{bin}_j \;\text{and}\; s > t \mid
             y = 1, E_{\text{true}} \in \text{bin}_i)$,
estimated from the same signal-cluster population and inverted with
Tikhonov regularisation ($\lambda = 10^{-2}$).  The resulting
$\Ntrue(E_{\text{true}})$ agrees with the direct
$E_{\text{pred}}$-binned correction to within $\pm 5\%$ on the
closure metric (actual deltas $-3.5$, $-4.0$, $-3.6$\,\% on
\texttt{zeroSpot}, \texttt{defaultSpot}, \texttt{bigSpot}
respectively; see
\texttt{closure\_unfold} column of
\texttt{sensitivity\_summary.csv}), providing a check that the
Wilson-uncertainty bookkeeping is not hiding a large migration bias.
Tightening $\lambda$ reduces the gap at the cost of amplifying
low-statistics-bin noise; the trade-off is quoted as a
regularisation systematic in Sec.~\ref{sec:systematics}.

We report both raw and efficiency-corrected yields per event
($\NperEv{\Nreco}$ and $\NperEv{\Ntrue}$) and overlay the per-event
MC-truth benchmark as a reference.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figs/sensitivity_yield}
...
\subsection{Per-cluster closure uniformity across three $\Usym$ samples}
\label{sec:closure}
...
\paragraph*{Two closure definitions.}
The per-cluster architecture of the reconstruction and the per-neutron
character of the physics observable make the closure metric
$\Ntrue/N_{\text{MC}}$ ambiguous.  We therefore quote both variants
explicitly and reconcile them via a physical multiplicative bridge:
\begin{align}
  C_\nu &\equiv \frac{\Ntrue}{N_{\text{MC}}^{\nu}}
    \qquad \text{(per-neutron closure)}, \label{eq:cnu}\\
  C_c &\equiv \frac{\Ntrue}{N_{\text{MC}}^{\text{cl}}}
    \qquad \text{(per-cluster closure)}, \label{eq:ccl}\\
  k   &\equiv \frac{N_{\text{MC}}^{\text{cl}}}{N_{\text{MC}}^{\nu}}
    \qquad \text{(cluster-per-neutron ratio)}, \label{eq:kratio}
\end{align}
with $C_\nu = k \cdot C_c$ by construction.
$N_{\text{MC}}^{\nu}$ is the count of unique MC neutron tracks
$(\text{event},\text{track})$ with $\texttt{n0\_label}=1$ passing the
detector fiducial (Sec.~\ref{sec:detector}); $N_{\text{MC}}^{\text{cl}}$
is the count of clusters flagged as signal by the truth-labeling
procedure of Sec.~\ref{sec:simulation}.  The ratio $k$ measures how
often multiple MC neutrons collapse into a single hit cluster:
$k = 1$ is the diagonal 1-neutron-per-cluster limit;
$k < 1$ signals multi-neutron cluster merging, which is a
neutron-density-dependent physical property of each dataset and
capping the per-cluster reconstruction from above.
Values of $C_c$ near unity indicate a well-calibrated
classifier + regression stage; the value of $C_\nu$ additionally
absorbs the merger term $k$.

Table~\ref{tab:closure} reports both metrics at threshold $t = 0.5$
on the full-statistics HPC-checkpoint sensitivity evaluation
($6.2$--$8.4 \times 10^{5}$ events per dataset in the seed-42
test half; all three datasets preprocessed at
$1.8$--$2.2 \times 10^{6}$ graphs each on cHARISMa without an
event cap).  The per-cluster closure is uniform across the three
datasets at $1.274 \pm 0.007$ (spread 1.0\%, $\pm 0.004$ at the
purity-locked working point of Table~\ref{tab:purity_locked}),
confirming that the classifier + regression pipeline is
dataset-independent at this scale.  All datasets over-recover
slightly ($C_c > 1$) because the efficiency correction inflates
true positives but does not subtract the small classifier
background leak; this bias is treated as a threshold-dependent
systematic in Sec.~\ref{sec:systematics} item \ref{sys:binning}.
The physical cluster-per-neutron ratio $k$ is also uniform to
$6.7\%$ at full statistics ($k = 0.669$--$0.714$; the smoke-scale
spread of $0.51$--$0.69$ turned out to be dominated by
small-sample fluctuation on the $\sim 1600$ smoke events per
dataset).  The per-neutron closure $C_\nu$ therefore inherits both
uniformities: at $t = 0.5$
$C_\nu = 0.889 \pm 0.025$ (spread 4.9\%), and at the purity-locked
working point $C_\nu = 1.020 \pm 0.030$ (spread 5.9\%), with the
$\Usym$-dependent multi-neutron cluster-merger accounting for
essentially all of the residual spread.  Physics-quality
sensitivity to $\Usym$ therefore rests on measuring
$k(\Usym)$ itself with statistical error, rather than on absorbing
it into a $C_\nu$ shift.  Two complementary follow-ups are deferred
to Sec.~\ref{sec:summary}:
(i) a differentiable clustering step that resolves multi-neutron
overlaps into separate cluster candidates, and
(ii) an unfolding pass over the response matrix
$M \otimes k^{-1}(\Usym)$ that treats cluster-merger as a
$\Usym$-dependent efficiency factor.

\begin{table}[!t]
  \centering
  \caption{Closure decomposition at $t = 0.5$, full-statistics
    HPC-checkpoint evaluation of the three \SMASH{} datasets on
    schema-v2 caches built on cHARISMa (preprocess job array 4284505
    for \texttt{zeroSpot}/\texttt{bigSpot} and 4290339 for the
    lift-cap \texttt{defaultSpot}; sensitivity job 4300982; each dataset
    $1.8$--$2.2 \times 10^{6}$ graphs, no per-dataset event cap).
    $N_{\text{ev}}$ is the per-dataset event count in the seed-42
    test half.  Both efficiency-basis conventions of
    Sec.~\ref{sec:sensitivity} are quoted; a Tikhonov detector-matrix
    unfold agrees with the $E_{\text{pred}}$-basis correction to
    $\pm 9\%$ (\texttt{closure\_unfold} column of
    \texttt{results/sensitivity\_full\_hpc\_finish\_4300982/sensitivity\_e\_pred/sensitivity\_summary.csv}).}
  \label{tab:closure}
  \begin{tabular}{lrrrrrr}
    \toprule
    dataset & $\Usym$ (MeV) & $N_{\text{ev}}$ & $k$ & $C_c$ (t=0.5) & $C_\nu$ (t=0.5) & $C_\nu = k C_c$ \\
    \midrule
    \texttt{zeroSpot}    & 0   & 640{,}333 & 0.714 & 1.270 & 0.907 & 0.907 \\
    \texttt{defaultSpot} & 18  & 838{,}284 & 0.669 & 1.282 & 0.858 & 0.858 \\
    \texttt{bigSpot}     & 90  & 619{,}603 & 0.712 & 1.269 & 0.903 & 0.903 \\
    \midrule
    max$-$min            &     &           & 0.045 & 0.013 & 0.049 &        \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figs/sensitivity_ratios}
...
The full-statistics sensitivity summary (per-event yields,
MC-truth, propagated uncertainties) is quoted in
Table~\ref{tab:closure} above; the raw and unfolded CSVs used to
populate it are catalogued in Sec.~\ref{sec:reproducibility}.
...
\begin{itemize}
  \item \textbf{Purity-locked benchmark.} Following
    ...
    efficiency vs.\ $\Ekin$ at the per-dataset threshold that yields
    a global signal purity of $\pi = 0.7$.  This is the operating
    point used for cross-checks against the rule-based cluster
    baseline of Sec.~\ref{sec:gnn:baseline}.  At full statistics
    the locked thresholds are $t^\star = 0.308 / 0.310 / 0.305$ for
    \texttt{zeroSpot} / \texttt{defaultSpot} / \texttt{bigSpot} and
    yield per-cluster closures $C_c = 1.457 / 1.466 / 1.461$ and
    per-neutron closures $C_\nu = 1.040 / 0.981 / 1.040$
    (Table~\ref{tab:purity_locked}), tightening the $C_c$ spread
    from $1.0\%$ at $t = 0.5$ to $0.62\%$ at the locked point while
    the $C_\nu$ spread stays within the $\sim 6.7\%$ physical
    $k(\Usym)$ ceiling of Sec.~\ref{sec:closure}
    (\texttt{scripts/purity\_locked\_closure.py}).
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figs/sensitivity_threshold_scan}
...
\begin{table}[!t]
  \centering
  \caption{Purity-locked closure at $\pi = 0.7$ per dataset,
    full-statistics HPC-checkpoint evaluation
    (job 4300982, post lift-cap defaultSpot preprocess 4290339).
    The $t^\star$ column is the per-dataset
    threshold that hits the target purity; $\varepsilon_{\text{cl}}$
    is the resulting cluster efficiency.  The per-cluster closure
    $C_c$ is uniform to $0.62\%$; the small residual per-neutron
    $C_\nu$ spread (0.059) is compatible with the $\sim 6.7\%$
    spread in $k(\Usym)$ (Table~\ref{tab:closure}).
    Source: \texttt{results/sensitivity\_full\_hpc\_finish\_4300982/purity\_locked\_pi70/purity\_locked\_pi70.csv}.}
  \label{tab:purity_locked}
  \begin{tabular}{lrrrr}
    \toprule
    dataset & $t^\star$ & $\varepsilon_{\text{cl}}$ & $C_c$ & $C_\nu$ \\
    \midrule
    \texttt{zeroSpot}    & 0.308 & 0.704 & 1.457 & 1.040 \\
    \texttt{defaultSpot} & 0.310 & 0.734 & 1.466 & 0.981 \\
    \texttt{bigSpot}     & 0.305 & 0.706 & 1.461 & 1.040 \\
    \midrule
    max$-$min            &        &        & 0.009 & 0.059 \\
    \bottomrule
  \end{tabular}
...
\begin{enumerate}
  \item \textbf{Cross-dataset feature-scaler drift.}
    ...
    fit by 285:1 in hit count ($10.3\times 10^{6}$ vs $36\times 10^{3}$
    per smoke dataset, see the top-of-run log of
    \texttt{run\_pooled\_scaler\_sensitivity.py}) so the pooled
    scaler is essentially identical to \texttt{defaultSpot}'s own
    (the 285:1 ratio applies only to the smoke-scale sample; at full
    statistics all three datasets contribute
    $\sim 1$--$1.4 \times 10^{9}$ hits and the pooled fit is
    balanced within a factor of two).  The residual per-neutron
    closure deficit on \texttt{defaultSpot} is therefore \emph{not}
    caused by scaler contamination; the binning basis
    (Sec.~\ref{sec:sensitivity}, item \ref{sys:binning} below)
    contributes $+2$--$4$\,pp, and the remaining spread is resolved
    by the $k(\Usym)$ decomposition of Sec.~\ref{sec:closure}.
  \item \textbf{Cluster-merging (per-neutron closure spread).}
    The per-neutron closure $C_\nu$ (Eq.~\ref{eq:cnu}) decomposes as
    $C_\nu = k \cdot C_c$ (Sec.~\ref{sec:closure}); at full statistics
    $C_c$ is uniform across the three datasets at $1.274 \pm 0.007$
    at $t = 0.5$ (Table~\ref{tab:closure}) while $k$ ranges
    $0.669$--$0.714$, so the observed $\sim 4.9\%$ spread in
    $C_\nu$ tracks the physical clusters-per-neutron ratio
    $k(\Usym)$ rather than any pipeline defect.  We quote both
    metrics and the multiplicative bridge, and treat
    $\text{spread}(k(\Usym))$ as the current systematic ceiling on
    any per-event yield-ratio interpretation.  Mitigation is
    deferred (differentiable clustering, or a
    $k$-conditional unfold --- Sec.~\ref{sec:summary}).
  \item \textbf{Per-cluster closure bias.}
    $C_c > 1$ across every dataset (Table~\ref{tab:closure}) because
    the efficiency correction inflates true positives without
    subtracting the small classifier background leak.  The bias is
    dataset-independent within 3\%; we quote the max
    $|C_c - 1|$ across thresholds $t \in \{0.3, 0.5, 0.7\}$ as a
    threshold-dependent systematic on $C_\nu$.  Per-bin, this bias
    concentrates at two locations visible in Fig.~\ref{fig:yield}:
    near $E_{\text{pred}} \approx 0.75$~GeV, where the classifier
    purity locally drops to $\sim 0.61$ due to background leakage
    across the $300$~MeV ToF threshold and the $1/\eff$ correction
    inflates the leak by a factor of $\sim 3.7$; and near
    $E_{\text{pred}} \approx 2.55$~GeV, where the energy regressor's
    modal migration toward the neutron spectrum peak accumulates
    events.  A signal-only correction (numerator restricted to
    $y = 1$ clusters) collapses the $0.75$~GeV bump from
    $C_{c,\text{bin}} = 1.70$ to $1.04$ and reduces the $2.55$~GeV
    bump from $1.81$ to $1.49$.  Both structures track across the
    three $\Usym$ samples to $\lesssim 11\%$ spread and therefore
    do not bias the cross-dataset closure metric of
    Sec.~\ref{sec:closure}.
  \item \textbf{Model-family disagreement.} Deferred.
    Full-statistics \texttt{hetero\_hgt} and \texttt{hetero\_sage}
    checkpoints have been trained (validation losses 0.464 and
    0.408 respectively on the shared \texttt{defaultSpot} recipe;
    \texttt{spectral\_dynedge} did not converge because of a
    LapPE-side CPU/CUDA device mismatch that remains open), but a
    per-cluster sensitivity comparison against \texttt{net\_default}
    is blocked by an eval-pipeline limitation: the shared
    \texttt{training.eval.predict} routine hardcodes the
    \texttt{net\_default} 6-tuple forward output
    $(\text{link}_s, \text{hit}_s, \text{cl}_s, E_{\text{pred}},
    \text{cl-link}_s, \text{cl-batch})$, while the \texttt{HeteroGNN}
    baseline returns a 2-tuple.  A per-\texttt{ModelSpec} output
    convention is needed before the model-family spread can be
    quoted; this refactor is scheduled as a follow-up.  Provisional
    ceiling: bounded above by the aggregated systematic on
    $C_\nu$ from the other items in this list.
  \item \label{sys:seed}\textbf{Seed variance.}
    Two additional training seeds ($s = 123, 456$) of
    \texttt{net\_default} were run under the same recipe as the
    2026-08-26 baseline ($s = 42$) and evaluated on the same
    full-statistics three-dataset trio.  The per-event yield ratio
    $R_d = \Ntrue(d)/\Ntrue(\texttt{defaultSpot})$ is reproducible
    to within
    $\max|R_{\text{zeroSpot}}| - \min|R_{\text{zeroSpot}}| = 0.007$
    (mean $\bar R_{\text{zeroSpot}} = 1.117$, $\sigma = 0.003$)
    and
    $\max|R_{\text{bigSpot}}| - \min|R_{\text{bigSpot}}| = 0.005$
    (mean $\bar R_{\text{bigSpot}} = 1.082$, $\sigma = 0.003$)
    across the three seeds (source:
    \texttt{results/systematics\_aggregate/seed\_variance.csv},
    aggregating cHARISMa jobs 4304929 and 4305276 on top of the
    baseline job 4300982).  Seed variance is therefore a $0.3\%$
    absolute contribution to the yield-ratio systematic budget ---
    an order of magnitude below the $k(\Usym)$ ceiling of item 2
    and negligible against the other sources in this list.
  \item \textbf{Threshold choice.}
    Included via the scan in Fig.~\ref{fig:threshold}.
  \item \label{sys:binning}\textbf{Efficiency binning basis and
        MC-truth definition.}
    Early smoke runs binned the efficiency correction $\eff$ by
    $E_{\text{true}}$ while $\Nreco$ was binned by $E_{\text{pred}}$;
    on the HPC checkpoint this mis-alignment produced a
    dataset-dependent 2–4\,\% shift in the closure metric.  The final
    pipeline bins $\eff$ by $E_{\text{pred}}$
    (Sec.~\ref{sec:sensitivity}), which restores closure by
    construction, and cross-checks the result with a detector-matrix
    unfold.  MC-truth is quoted per-neutron (unique
    \texttt{(Row, Id)} tracks with $\texttt{n0\_label}=1$) throughout;
    the per-cluster convention ($\texttt{cl\_label}=1$) is used
    internally for efficiency estimation only.
...
(i) a differentiable clustering step to replace the fixed
$r_{\text{loc}}, \Delta t$ radius-graph, so that multi-neutron
overlaps within a single cluster (which cap $C_\nu$ at $k$;
Sec.~\ref{sec:closure}) are resolved into per-neutron candidates;
(ii) equivalently, a joint unfolding pass over the response matrix
and the $\Usym$-dependent cluster-per-neutron ratio $k(\Usym)$ so
that the physically-quoted per-neutron closure recovers unity across
datasets;
...
The complete analysis pipeline (preprocessing, training, evaluation,
sensitivity CLI, figure production, and this manuscript source) is
available at \url{https://github.com/voboch/HGNDRecoGNN} under the
tag \texttt{v1.0-prc-submission}.  The end-to-end runbook is in
\texttt{PIPELINE.md}; regression tests guarding the analysis-CLI
conventions are in \texttt{tests/}.  The HPC checkpoint used
throughout Sec.~\ref{sec:performance}--\ref{sec:sensitivity}
(\texttt{net\_default}, $H=512$, $L=8$, 20 epochs; SHA-256
\texttt{f8647e17\dots}) is deposited alongside the code with the
preprocessed schema-v2 shard \texttt{meta.json} manifests.
Raw \DCM{} and \SMASH{} event dumps are available on request from
the authors, subject to the caveat in Sec.~\ref{sec:simulation} about
the SMASH-side simulation issue identified after this analysis was
frozen.
...
\url{https://github.com/voboch/HGNDRecoGNN} (tag
\texttt{v1.0-prc-submission}).
The end-to-end runbook (raw data $\to$ preprocess $\to$ train $\to$
eval $\to$ sensitivity CLI $\to$ paper tables) is documented in
\texttt{PIPELINE.md} of the repository; five regression tests
(\texttt{tests/test\_sensitivity\_pipeline.py}) guard the
sensitivity-CLI conventions used by
...
\begin{itemize}
  ...
  \item \emph{Cluster (SLURM/CUDA).}
    ...
    \texttt{slurm/sensitivity\_full.hpc.sbatch} on the cHARISMa
    \texttt{rocky} partition with a single V100 (32 GB) GPU node
    (\texttt{--constraint=type\_a}); typical wall time is 3--5~h
    for the combined evaluate + sensitivity CLI + purity-lock
    pipeline (reference jobs 4284506 and 4300982).  Preprocessing
    is a CPU-only companion via
    \texttt{slurm/preprocess\_all\_smash.hpc.sbatch} (2--3~h per
    dataset on 16 workers).
\end{itemize}
...
\bibitem{LiChen2008SymEnergy}
  B.-A.~Li, L.-W.~Chen, and C.~M.~Ko,
  ``Recent progress and new challenges in isospin physics with
   heavy-ion reactions,''
  Phys.~Rep.~\textbf{464}, 113 (2008).

...
$C_c$ is uniform across the three samples to $1.4\%$ at $t = 0.5$
and $0.75\%$ at the purity-locked point.
...
on the full-statistics HPC-checkpoint sensitivity evaluation (~76k
to 640k events per dataset in the seed-42 test half; caveat on the
100k-event preprocessing cap on \texttt{defaultSpot} noted in the
table caption and Sec.~\ref{sec:reproducibility}).  The per-cluster
closure is uniform across the three datasets at $1.275 \pm 0.010$
(spread 1.4\%, $\pm 0.006$ at the purity-locked working point of
Table~\ref{tab:purity_locked}), confirming that the classifier +
regression pipeline is dataset-independent at this scale.  All
datasets over-recover slightly ($C_c > 1$) because the efficiency
correction inflates true positives but does not subtract the small
classifier background leak; this bias is treated as a
threshold-dependent systematic in Sec.~\ref{sec:systematics} item
\ref{sys:binning}.  The physical cluster-per-neutron ratio $k$ is
also uniform to 6.5\% at full statistics
($k = 0.670$--$0.714$; the smoke-scale spread of 0.51--0.69 turned
out to be dominated by small-sample fluctuation on the $\sim 1600$
smoke events per dataset).  The per-neutron closure $C_\nu$
therefore inherits both uniformities: at $t = 0.5$
$C_\nu = 0.891 \pm 0.023$ (spread 4.5\%), and at the purity-locked
working point $C_\nu = 1.021 \pm 0.032$ (spread 5.6\%), with the
combined $\Usym$-dependent multi-neutron cluster-merger accounting
for essentially all of the residual spread.  Physics-quality
...
\begin{table}[!t]
  ...
  \caption{Closure decomposition at $t = 0.5$, full-statistics
    ...
    schema-v2 caches built on cHARISMa (preprocess array 4284505,
    sensitivity job 4284506; \texttt{zeroSpot}
    $1.86 \times 10^{6}$ graphs, \texttt{bigSpot} $1.80 \times 10^{6}$
    graphs, \texttt{defaultSpot} $2.0 \times 10^{5}$ graphs --- the
    latter still under the $100\,000$-event preprocessing cap from
    2026-08-26, to be lifted before the final table).  $N_{\text{ev}}$
    is the per-dataset event count in the seed-42 test half.  Both
    efficiency-basis conventions of Sec.~\ref{sec:sensitivity} are
    quoted; a Tikhonov detector-matrix unfold agrees with the
    $E_{\text{pred}}$-basis correction to $\pm 5\%$
    (\texttt{closure\_unfold} column of
    \texttt{results/sensitivity\_full\_hpc\_4284506/sensitivity\_e\_pred\_fixed/sensitivity\_summary.csv}).}
  \label{tab:closure}
  \begin{tabular}{lrrrrrr}
    ...
    \texttt{defaultSpot} & 18  &  76{,}600 & 0.670 & 1.286 & 0.862 & 0.862 \\
    \texttt{bigSpot}     & 90  & 619{,}603 & 0.712 & 1.269 & 0.903 & 0.903 \\
    \midrule
    max$-$min            &     &           & 0.044 & 0.017 & 0.045 &        \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Purity-locked benchmark.} Following
    ...
    yield per-cluster closures $C_c = 1.457 / 1.468 / 1.461$ and
    per-neutron closures $C_\nu = 1.040 / 0.984 / 1.040$
    (Table~\ref{tab:purity_locked}), tightening the $C_c$ spread
    from $1.4\%$ at $t = 0.5$ to $0.75\%$ at the locked point while
    the $C_\nu$ spread stays within the $\sim 4.4\%$ physical
...
\begin{table}[!t]
  ...
  \caption{Purity-locked closure at $\pi = 0.7$ per dataset,
    ...
    (job 4284506).  The $t^\star$ column is the per-dataset
    threshold that hits the target purity; $\varepsilon_{\text{cl}}$
    is the resulting cluster efficiency.  The per-cluster closure
    $C_c$ is uniform to $0.75\%$; the small residual per-neutron
    $C_\nu$ spread (0.056) is compatible with the $\sim 4.4\%$
    spread in $k(\Usym)$ (Table~\ref{tab:closure}) and no longer
    dominates.
    Source: \texttt{results/sensitivity\_full\_hpc\_4284506/purity\_locked\_pi70\_fixed/purity\_locked\_pi70.csv}.}
  ...
  \begin{tabular}{lrrrr}
    ...
    \texttt{defaultSpot} & 0.310 & 0.731 & 1.468 & 0.984 \\
...
\begin{enumerate}
  ...
  \item \textbf{Cluster-merging (per-neutron closure spread).}
    ...
    $C_c$ is uniform across the three datasets at $1.275 \pm 0.010$
    at $t = 0.5$ (Table~\ref{tab:closure}) while $k$ ranges
    $0.670$--$0.714$, so the observed $\sim 4.5\%$ spread in

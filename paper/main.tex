...
Tikhonov regularisation.  The resulting $\Ntrue(E_{\text{true}})$
agrees with the direct $E_{\text{pred}}$-binned correction to
$\pm 3\%$ on the closure metric, providing a check that the
...
\begin{table}[!t]
  ...
  \caption{Sensitivity summary at threshold $t = 0.5$ (placeholder
    ...
    closure is the per-neutron $C_\nu = \Ntrue/N_{\text{MC}}^{\nu}$
    (Eq.~\ref{eq:cnu}).  The per-cluster $C_c$ (Eq.~\ref{eq:ccl}) and
    cluster-per-neutron ratio $k$ (Eq.~\ref{eq:kratio}) are quoted
    separately in Table~\ref{tab:closure}.}
  \label{tab:summary}

...
On the \DCM{} test split the checkpoint (\texttt{net\_default},
$H=512$, $L=8$, 20 epochs) reaches cluster ROC AUC 0.944 and, at a
signal-purity-locked working point of $\pi = 0.702$
($t^\star = 0.305$), cluster efficiency 0.739 and per-event neutron
detection efficiency 0.844 (Table~\ref{tab:eff_ekin}), matching the
prototype benchmarks of Ref.~\cite{Morozov2024HGND}; the relative
energy resolution is below 10\% for $0.7 \lesssim \Ekin \lesssim 5$~GeV.
...
and $0.48\%$ at the purity-locked point.
...
At the purity-locked operating point $t \approx 0.31$ used for
comparison with the rule-based baseline of
Sec.~\ref{sec:gnn:baseline} (purity $\pi = 0.706$),
$\varepsilon_{\text{cl}} = 0.739$ and $\varepsilon_{\text{ev}} = 0.844$
(purity $0.702$, $t^\star = 0.305$),
matching the value reported at the \BMN{} analysis-and-detector
meeting~\cite{Bocharnikov2026Dombay}.  All quantities are computed
...
\begin{itemize}
  \item \textbf{Purity-locked benchmark.} Following
    ...
    yield per-cluster closures $C_c = 1.457 / 1.454 / 1.461$ and
    per-neutron closures $C_\nu = 1.040 / 0.981 / 1.040$
    (Table~\ref{tab:purity_locked}), tightening the $C_c$ spread
    from $1.0\%$ at $t = 0.5$ to $0.48\%$ at the locked point while
...
\begin{table}[!t]
  ...
  \caption{Purity-locked closure at $\pi = 0.7$ per dataset,
    ...
    $C_c$ is uniform to $0.48\%$; the small residual per-neutron
  ...
  \begin{tabular}{lrrrr}
    ...
    \texttt{defaultSpot} & 0.310 & 0.734 & 1.454 & 0.981 \\

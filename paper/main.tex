...
signal-purity-locked working point of $\pi = 0.706$, cluster
efficiency 0.735 and per-event neutron detection efficiency 0.841
(Table~\ref{tab:eff_ekin}), matching the prototype benchmarks of
Ref.~\cite{Morozov2024HGND}; the relative energy resolution is
below 10\% for $0.7 \lesssim \Ekin \lesssim 5$~GeV.
As a pipeline-calibration cross-check we evaluate the same
...
\begin{table}[!t]
  ...
  \caption{Closure decomposition at $t = 0.5$, full-statistics
    ...
    schema-v2 caches built on cHARISMa (job 4284505; \texttt{zeroSpot}
...
The full-scale sensitivity summary (per-event yields, MC-truth,
propagated uncertainties) is deferred to the SLURM sweep of
Sec.~\ref{sec:reproducibility}; at smoke scale the corresponding
numbers are quoted in Table~\ref{tab:closure} above.
...
\begin{itemize}
  \item \textbf{Purity-locked benchmark.} Following
    ...
    baseline of Sec.~\ref{sec:gnn:baseline}.  At smoke scale the
    locked thresholds are $t^\star = 0.333 / 0.303 / 0.313$ for
    \texttt{zeroSpot} / \texttt{defaultSpot} / \texttt{bigSpot} and
    yield per-cluster closures $C_c = 1.44 / 1.40 / 1.47$ and
    per-neutron closures $C_\nu = 0.967 / 0.736 / 0.980$
    (Table~\ref{tab:purity_locked}).  We note as a negative result
    that the purity lock does \emph{not} reduce the $C_\nu$ spread
    across the three datasets --- max$-$min widens from 0.206 at
    $t = 0.5$ to 0.244 at the locked point --- consistent with the
    interpretation of Sec.~\ref{sec:closure} that the spread is
    driven by the physical $k(\Usym)$ rather than by the classifier
    working point (\texttt{scripts/purity\_locked\_closure.py}).
...
\begin{enumerate}
  ...
  \item \textbf{Cluster-merging (per-neutron closure spread).}
    ...
    $C_\nu = k \cdot C_c$ (Sec.~\ref{sec:closure}); at smoke scale
    $C_c$ is uniform across the three datasets at $1.25 \pm 0.03$
    while $k$ ranges 0.51--0.69, so the observed 0.20 spread in
    $C_\nu$ is dominated by the physical clusters-per-neutron ratio
    $k(\Usym)$ rather than by any pipeline defect.  We quote both

...
Key numerical results on the HPC-trained checkpoint
(\texttt{net\_default}, $H=512$, $L=8$, 20 epochs):
per-event neutron detection efficiency $\eff_{\text{ev}} = 0.841$ at
locked signal purity $\pi = 0.706$ (cluster efficiency 0.735;
cluster ROC AUC 0.944); energy resolution below 10\% for
$0.7 \lesssim \Ekin \lesssim 5$~GeV.  At full statistics
($\sim 10^{5}$--$6 \times 10^{5}$ events per dataset in the seed-42
test half) the three \SMASH{} samples show per-cluster closure
$C_c = 1.275 \pm 0.010$ (spread 1.4\%) at $t = 0.5$ and
$C_c = 1.462 \pm 0.006$ (spread 0.75\%) at the purity-locked working
point, uniform across $\Usym$; the per-neutron closure
$C_\nu = 0.891 \pm 0.023$ (spread 4.5\%) is compatible with the
$\sim 4.4\%$ physical spread in the clusters-per-neutron ratio
$k(\Usym) = 0.670$--$0.714$ (Sec.~\ref{sec:closure}).
\end{abstract}

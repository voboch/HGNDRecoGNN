...
of $\pi = 0.702$ ($t^\star = 0.305$), cluster efficiency 0.739 and
...
\begin{table}[!t]
  ...
  \caption{Closure decomposition at $t = 0.5$, full-statistics
    ...
    schema-v2 caches built on cHARISMa (preprocess jobs 4284505
    and 4290339, sensitivity jobs 4284506 and 4300982; each dataset
...
\begin{enumerate}
  \item \textbf{Cross-dataset feature-scaler drift.}
    ...
    The residual per-neutron closure deficit on \texttt{defaultSpot}
    is therefore \emph{not} caused by scaler contamination; the
    binning basis (Sec.~\ref{sec:sensitivity}, item \ref{sys:binning}
    below) contributes $+2$--$4$\,pp, and the remaining spread is
    resolved by the $k(\Usym)$ decomposition of
    Sec.~\ref{sec:closure}.
